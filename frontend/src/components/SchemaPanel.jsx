...
export default function SchemaPanel({ schema, onSampleQuery }) {
  ...
  const samples = [
    ...
    { label: 'Insert a customer', sql: `INSERT INTO Customers (customer_id, first_name, last_name, age, country)\nVALUES (6, 'Sara', 'Ali', 27, 'Kuwait');` },
    ...
    <div className="schema-panel">
      ...
      <section className="schema-section">
        ...
        <ul className="samples-list">
          {samples.map((s) => (
            <li key={s.label}>
              <button className="sample-btn" onClick={() => onSampleQuery(s.sql)}>
                {s.label}
              </button>
            </li>
          ))}
